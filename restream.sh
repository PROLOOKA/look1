...
# دالة لإعادة المحاولة إذا توقف ffmpeg
run_ffmpeg() {
    ffmpeg -re -user_agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" -i "$SOURCE_URL" -i logo.png \
    -filter_complex \
...
# تشغيل ffmpeg وإعادة تشغيله تلقائياً إذا توقف
while true; do
